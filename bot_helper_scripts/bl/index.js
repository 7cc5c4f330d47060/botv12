...
const tth=function(T,go){
	...
	if(T){
			...
			if(T.color=="reset")		{areturn+="\x1b[0m\x1b[1m\x1b[37m";			breturn+="\u00a7r";pc=T.color} else if(!T.color && !go){areturn+="\x1b[0m\x1b[1m\x1b[37m";breturn+="\u00a7r"}
			/**/
	...
	if(T.selector){
		...
		if (T.extra){
			for(var iza=0; iza<=T.extra.length-1; iza++){
				areturn+= tth(T.extra[iza],go)[0]
				breturn+= tth(T.extra[iza],go)[1]
				creturn+= tth(T.extra[iza],go)[2]
			}
		}
