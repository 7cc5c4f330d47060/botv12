...
const trans4_1 = require('./translations.json')
let trans4 = Object.create(null) // block __proto__ and constructor
for (const i in trans4_1){
  trans4[i] = trans4_1[i]
}
...
const cp = function (c,oc,level) {
  if(!level) level=0;
  if(level>=10){ //crappy way of preventing errors
  ...
  if (c.startsWith('#')) {
    ...
    textConsole += '\x1b[0m\x1b[38;5;15m'
    textMC += '\xa7f'
  } else if (c == 'reset' && oc!='reset') {
    color0 = cp(oc, null, level+1);
    textConsole += color0[0];
    textMC += color0[1];
  } else {
    textConsole += '\x1b[0m\x1b[38;5;15m'
    textMC += '\xa7r'
...
const j = function (msg, l, color0) {
  ...
  if (msg.translate) {
    ...
    for (const i in msg.with) {
      const j2 = j(msg.with[i], l + 1, msg.color ? msg.color : 'white')
    ...
    if (msg.with) {
      if (msg.with[0]) {
        const j2_1 = j(msg.with[0], l + 1, msg.color ? msg.color : 'white')
      ...
      if (msg.with[1]) {
        const j2_2 = j(msg.with[1], l + 1, msg.color ? msg.color : 'white')
      ...
      if (msg.with[2]) {
        const j2_3 = j(msg.with[2], l + 1, msg.color ? msg.color : 'white')
      ...
      if (msg.with[3]) {
        const j2_4 = j(msg.with[3], l + 1, msg.color ? msg.color : 'white')
  ...
  if (msg.extra) {
    for (const i in msg.extra) {
      if (msg.color) {
        ...
        const c2 = cp('reset',color0)
        textConsole += c2[0]
        textMC += c2[1]
      }
      const j2 = j(msg.extra[i], l + 1, msg.color ? msg.color : 'white')
      textClear += j2[0]
      textConsole += j2[1]
      textMC += j2[2]
    }
  ...
  textClear = textClear.replace(/§[0-9a-fk-or]/g, '') // v9 first change (delete this comment when release)
  textConsole = textConsole.replace(/§[0-9a-fk-or]/g, '')
  textMC = textMC.replace(/§([0-9a-frx#])(?=§([0-9a-frx#]))/g, '')//.replace(/§([0-9a-frx]|#[0-9a-f])(?=§([0-9a-frx]|#[0-9a-f]))/g, '')
...
module.exports = {
  ...
  load2: function (b) {
    ...
    b.on('system_chat', function (MSG) {
      ...
      if (slashsay) {
      ...
      } else if (JSON.parse(MSG.content).translate == '[%s] %s › %s' || JSON.parse(MSG.content).translate == '%s %s › %s') {
        const split = JSON.parse(MSG.content)
        if (split.with === undefined || split.with.length < 2) return
        ...
        b.emit('playermsg', command_full, uuid, username, command_full)// MSG.sender)
      } else {
        ...
        b.emit('playermsg', command_full_c, uuid, username2 || usernameClear, command_full)// MSG.sender)
      }
    })
  },
  parse: j
