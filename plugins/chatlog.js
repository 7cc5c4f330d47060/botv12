...
const checkLog = () => {
  ...
  try {
    ...
    if (!fs.existsSync(`./logs/${filenameToday}`)) fs.mkdirSync(`logs/${filenameToday}`)
    if (!fs.existsSync(`./logs/${filenameTomorrow}`)) fs.mkdirSync(`logs/${filenameTomorrow}`) // Create tomorrow's log directory early
  } catch (e) {
