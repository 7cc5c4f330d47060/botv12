...
//console.log(new Command(1,1,1,1,1,1))
console.log()
const fs = require('fs')
// index.p.cmdloader=1;
const loadplug_new = () => {
  const botplug = Object.create(null) // block __proto__ and constructor
...
const loadplug = () => {
  const botplug = Object.create(null) // block __proto__ and constructor
  const bpl = fs.readdirSync('new_command')
  let id = 0
  for (const i in bpl) {
    if (!bpl[i].endsWith('.js')) {
      continue
    ...
    try {
      const name = bpl[i].slice(0, bpl[i].length - 3)
      botplug[name] = require(`../new_command/${bpl[i]}`)
      botplug[name].id = id++
      if (botplug[name].aliases) {
        for (const j in botplug[name].aliases) {
          botplug[botplug[name].aliases[j]] = {}
...
module.exports = {
  ...
  load2: function (b) {
    ...
    b.rc = function (cmdFormat, sender, user, cmd) {
      ...
      try {
        if (module.exports.newercommands[command]) {
          if (module.exports.newercommands[command].delay && b.cmddelay[command] && Date.now() - b.cmddelay[command] <= module.exports.newercommands[command].delay && !b.o.cc_enabled) {
            b.message(`You must wait ${Math.floor((b.cmddelay[command] - Date.now() + module.exports.newercommands[command].delay) / 1000)} more second${Math.floor((b.cmddelay[command] - Date.now() + module.exports.newercommands[command].delay) / 1000) == 1 ? '' : 's'} to run this command again.`)
            return
          } else {
            b.cmddelay[command] = Date.now()
          }
          // if(module.exports.commands[command].verify && sender!="00000000-0000-0000-0000-Console00000"){
          if (!b.autoVerify && args[args.length - 1] != genCode(user) && module.exports.newercommands[command].verify) {
            b.message('Invalid hash')
            return
          } else if (args[args.length - 1] == genCode(user) || b.autoVerify) {
            v = true
          }
          if (module.exports.newercommands[command].coreRequired && !b.o.cc_enabled) {
            b.send('Command core is disabled, command will not run.')
          } else if (module.exports.newercommands[command].opRequired && (b.o.partial_op || b.o.deop)) {
            b.send('OP access is disabled, command will not run.')
          } else {
            const command_c = new Command(sender,user,user,module.exports.newercommands[command].format ? cmdFormat : cmd,b.prefix,b,v)
            module.exports.newercommands[command].command(command_c)
          }
          fs.appendFileSync(log_filename(Date.now(), b.host, b.port, 'cmd'), `${log_date()} ${user} (${sender}): ${cmd}\n`)
        } else if (module.exports.newcommands[command]) {
        ...
        } //new Command("00000000-0000-0000-0000-000000000000","username","nickname","help",'"',index.bots[0])
      } catch (err) {
        b.send(err + '')
      }
