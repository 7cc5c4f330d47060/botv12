...
if((n=="Zelkam" || n=="ZelkTheElk") && c.toLowerCase(0).split(" ")[0] == "zelkam"){ cwc(csl[1]+n+csl[0]+" is not allowed to run "+csl[1]+"|"+c.split(" ")[0]+csl[0]+"."); return;}
	...
	tc: {
		command: function(c,n){
			if(cdtc<=0){
			client.write("tab_complete",{transactionId:618387,text:c.slice(3)});cdtc=10;}else{
	...
	srvr: {
		command: function(c,n){
			mc.ping({host:c.split(" ")[1],port:c.split(" ")[1]},function(texter){try{return cwc(lang.tth(JSON.parse(texter).description).split("\u00a7").join("&"))}catch(er2a){cwc("Error")})
		},
		perm: 0,
		admin: 0,
		confirm:0
...
cwc("/tp 0 300 10")
setTimeout(function(){setInterval(function(){antiBotBypass();},500)},2950)
cwc("/cspy off")
cwc("/skin SkeppyCat")
cwc("/v")
cwc("/team leave @s")
